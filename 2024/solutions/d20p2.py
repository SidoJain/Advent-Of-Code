...
def find_cheats(maze: np.ndarray, start_dists: np.ndarray, end_dists: np.ndarray, start_pos: tuple[int, int], max_cheat_time: int) -> dict[int, int]:
    height, width = maze.shape
    ...
    for r, c in zip(*valid_positions):
        ...
        for dr in range(-max_cheat_time, max_cheat_time + 1):
            for dc in range(-max_cheat_time, max_cheat_time + 1):
                ...
                total_time = time_to_start + dist + end_dists[nr, nc]
                time_saved = normal_time - total_time
                if time_saved > 0:
                    cheats[time_saved] += 1
    return cheats

def count_good_cheats(maze_str: str, min_time_saved: int, max_cheat_time: int) -> int:
